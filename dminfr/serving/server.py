...
Backends:
  - "ours"        : Dense baseline (dminfr.reference.generate)
...
Usage:
    python3 -m dminfr.serving.server --weight-dir ./weights --port 8000 --backend fast_dense
"""
...
# dminfr/engine/model.py supports today (F.scaled_dot_product_attention is
...
# benchmarks/check_time_inference.py --batch-size {1,32,64} under Nsight Compute
...
# (benchmarks/throughput/run_throughput.py --concurrency 64 --n-requests 128):
...
async def _raise_threadpool_limit():
    ...
    (e.g. 64)."""
    from dminfr.engine.distributed import get_tp_size
...
class ChatRequest(BaseModel):
    ...
    # Opt-in: use dminfr/engine/generate.py's generate_dense (same model
    ...
    if BACKEND == "fast_dense" and get_tp_size() == 1:
        ...
        with torch.no_grad(), request_lock:
            if BACKEND == "ours":
                ...
                from dminfr.engine.model import LLaDAMoEKV
...
def load_model(weight_dir: str, device: str, backend: str):
    ...
    if backend == "ours":
        from dminfr.reference.model import LLaDAMoE, load_weights
        ...
        from dminfr.engine.distributed import load_weights_tp, get_tp_rank
...
def main():
    from dminfr.engine.distributed import init_distributed, get_tp_rank, get_tp_size
    init_distributed()
