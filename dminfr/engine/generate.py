...
def get_num_transfer_tokens(mask_index: torch.Tensor, steps: int) -> torch.Tensor:
    ...
    same class of cost tests/test_moe_align_block_size.py's vectorization
...
def select_transfer_indices(confidence: torch.Tensor, num_transfer_step: torch.Tensor) -> torch.Tensor:
    ...
    for that row, not an approximation (see tests/test_select_transfer_indices.py).
...
def generate_cached(
    ...
    lets a batch mix prompt lengths -- without it, `dminfr/serving/server.py` can only
...
def generate_dense(
    ...
    dminfr.reference.generate.generate()'s and HF's reference algorithm, just through
