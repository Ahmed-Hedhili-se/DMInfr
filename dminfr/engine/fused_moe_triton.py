        ...
        try:
            ...
            if not is_keyed and torch.cuda.is_available():
                print(f"Warning: using unkeyed {os.path.basename(path)}; it may have "
                      ...
                      f"Re-run dminfr.tuning.autotune_moe.py to produce a device-keyed file.")
            return cfg
...
# tests/test_fused_silu_epilogue.py uses, and the escape hatch if a future
...
def fused_moe_kernel(
    ...
    "almost always" is not "never" -- tests/test_fused_silu_epilogue.py
...
def moe_align_block_size(topk_ids: torch.Tensor, block_size: int, num_experts: int):
    ...
    input (see tests/test_moe_align_block_size.py, which checks this
...
def fused_moe(hidden_states: torch.Tensor, w1: torch.Tensor, w2: torch.Tensor,
    ...
    True/False force it, which is how tests/test_fused_silu_epilogue.py A/Bs the
