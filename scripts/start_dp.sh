...
# dminfr/serving/server.py disables request batching whenever tp_size > 1 -- a TP=8 server
...
# Every replica runs `-m dminfr.serving.server`, which needs the repo root on Python's
...
# from REPO_ROOT above) so only the replica launch's CWD was ever missing.
cd "$REPO_ROOT"
...
for ((i = 0; i < REPLICAS; i++)); do
    ...
    #
    # Each replica needs its OWN rendezvous port. dminfr.serving.server calls
...
"$PY" -m dminfr.serving.router --backends "$BACKENDS" --port "$PORT" --host 0.0.0.0 &
ROUTER_PID=$!
PIDS+=("$ROUTER_PID")
wait "$ROUTER_PID"
