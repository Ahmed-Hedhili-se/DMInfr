# `dminfr.engine` Optimization & Correctness Investigation Log

Full chronological record of what was tried on `dminfr.engine` (the
...
1. **Expert-count reduction** (dynamic top-k, then adaptive/nucleus top-k) —
   ...
   reverted.
2. **`dminfr.engine` vs HF correctness gap** — a real accuracy bug found in
...
`dminfr/engine/generate.py`: every token in a given denoising step used the
...
`dynamic_k`/`nucleus_p` removed from `dminfr/engine/model.py` and
...
## Part 2 — `dminfr.engine` vs HF correctness gap: the KV-cache collapse bug

With expert-count reduction abandoned and `dminfr.engine` left running
native static top-8, the next step was validating `dminfr.engine`'s output
...
Running the correctness harness (`benchmarks/correctness/run_correctness.py`)
with CoT prompting on identical, same-seeded MMLU / MMLU-Pro subsets:

| Backend | MMLU | MMLU-Pro |
|---|---|---|
| `dminfr.engine` (`fast_dense`) | 59.0% | 28.0% |
...
`dminfr.engine`'s CoT responses collapsed to a bare
...
|---|---|---|
| `dminfr.engine` | 36/50 (72%) | 22/50 (44%) |
| HF | 0/50 | 0/50 |
...
1. **Cached** — `dminfr.engine.generate.generate_cached` (the production
   path).
...
to `dminfr.engine`'s KV-cache / block-wise generation logic
...
Changed in `dminfr/engine/generate.py` (commit `a5f6ebe`):
...
(`benchmarks/correctness/run_correctness.py --task mmlu_pro --seed 42 --limit 50`,
same config used throughout this investigation) against `dminfr.engine`
post-fix:

| Backend | MMLU-Pro (n=50) |
|---|---|
| `dminfr.engine`, pre-fix | 28.0% |
| `dminfr.engine`, post-fix | **40.0%** |
| HF reference | 46.0% |
...
section for the surrounding context), the same `dminfr.engine`-vs-HF
...
|---|---:|
| `dminfr.engine` (`fast_dense`, chat-template, no threshold) | 68.0% (34/50) |
...
| Response length (chars) | n | `dminfr.engine` acc | HF acc | gap |
|---|---:|---:|---:|---:|
...
+16.7pt → +50.0pt) — `dminfr.engine` is not uniformly worse than HF, it is
...
(1200+ chars), `dminfr.engine` got **zero correct** while HF got half
...
`dminfr/engine/generate.py` gained `generate_dense` (same model class/
...
really the cause, disabling caching should move `dminfr.engine`'s accuracy
toward HF's 74.0%.
...
`dminfr.engine` scoring 0/6 on 1200+ char responses) was real data, but
...
`dminfr.engine`'s design matches dInfer's `prefix` type, and that dInfer's
...
`dminfr.engine` exactly. No algorithmic gap.
...
(dminfr.engine uses `F.scaled_dot_product_attention`; the HF comparison
...
runs. The full matrix (`--eager-moe` swaps in dminfr.engine's eager
...
training numerics behaves most as-trained." Either way, dminfr.engine is
...
| `dminfr.engine` vs HF correctness testing | Compare CoT MMLU/MMLU-Pro accuracy | Found real, large gap (28% vs 46% MMLU-Pro) caused by response collapse to bare `"Final Answer: X"` |
